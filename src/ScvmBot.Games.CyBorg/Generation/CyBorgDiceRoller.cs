...
public sealed class CyBorgDiceRoller
{
    private readonly Random _rng;

    public CyBorgDiceRoller(Random rng)
    {
        _rng = rng;
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(die)) return 6;
        var numeric = die.TrimStart('d', 'D');
        return int.TryParse(numeric, out var size) && size > 0 ? size : 6;
    }

    public int RollCredits(Reference.CyBorgCreditsFormula formula)
    {
        var sum = 0;
